...
const Navbar = () => {
	...
	return (
		<div className='fixed w-full z-10'>
			<header>
				<nav  className={`flex justify-between h-20 w-full bg-navbar-background/10 backdrop-blur-md px-5 md:px-10 lg:px-20 items-center border-b-0 ${navbarScroll && 'shadow-navbar'}`}>
					...
					<ul className='flex justify-between items-center'>
						...
						<li className={`hidden md:flex text-lg font-semibold mx-5 p-3 cursor-pointer ${getLinkClass('/projects')}`}>
							<Link href="/projects">
								Projects
							</Link>
						</li>

						<li className={`hidden md:flex text-lg font-semibold mx-5 p-3 cursor-pointer ${getLinkClass('/about')}`}>
							<Link href="/about">
								About
							</Link>
						</li>
				...
				{/* {nav && ( */}
					<div className={`${nav ? 'z-5 fixed w-full h-auto top-20 bg-background p-4 shadow-navbar ease-in-out duration-800 md:hidden' : 'hidden fixed -top-full'}`}>
